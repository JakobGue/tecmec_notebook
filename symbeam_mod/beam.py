...
from symbeam_mod.load import distributed_load, point_load, point_moment
from symbeam_mod.point import continuity, fixed, hinge, pin, roller
...
class beam:
    ...
    # ------------------------------------------------------------------------- set_segments
    def _set_segments(self, print_segments=False):
    ...
    # ---------------------------------------------------------------------- solve_reactions
    def _solve_reactions(self, show_calculation=False):
        ...
        # Reaction forces first.
        for i in range(len(self.points)):
            if self.points[i].has_reaction_force():
                name = "F{0}".format(chr(ord('@')+i))
        ...
        if len(equilibirum_equations) != len(unknown_reactions):
            raise RuntimeError(
                ...
                + "and unknwown reactions does not match. Check the implementation of "
                + "the supports."
            )
        if show_calculation:
    ...
    def _solve_internal_loads(self, show_calculation=False):
        """Finds the shear force and bending moment diagrams for each segment of the beam by
        integrating the differential equilibrium relations.
        """
        ...
        # Loop over the segments and find the shear force and bending moment distribution.
        if show_calculation:
            print("="*30)
            print("Equilibrium equations for each segment:")
        for i in range(len(self.segments)):
            ...
            segment_dist_loads = [x for x in self.distributed_load_list if x.expression != 0 and x.x_start < self.segments[i].x_end ]
            segment_points = [x for x in self.points if x.x_coord < self.segments[i].x_end and x.reaction_force != 0 or x.external_force != 0]
            
            equation_str = []
            equation = sym.sympify(sym.symbols('Fq{}'.format(i+1)))
            for load in [*segment_dist_loads, *segment_points]:
                
                if type(load) == distributed_load:
                    if load.expression != 0:
                        equation_str.append(((sym.symbols('x') - (load.x_end - load.x_start)) * sym.symbols('q')))
                        equation = equation + ((sym.symbols('x') - (load.x_end - load.x_start)) * load.expression)
                
                elif type(load) in (continuity, fixed, hinge, pin, roller):
                    if load.reaction_force != 0:
                        equation_str.append(load.reaction_force)
                        equation = equation + load.reaction_force
                    if load.external_force != 0:
                        equation_str.append(load.external_force)
                        equation = equation + load.external_force

            
            equation_str = [str(x) for x in equation_str]
            equation_str = ' + '.join(equation_str) + ' = -Fq{}'.format(i+1)
            equation_str = equation_str.replace('+ -', '- ')
                    
            if show_calculation:
                print(20*'-')
                print(equation_str)
                print(f'Fq{i+1} = {sym.solve(equation, sym.symbols("Fq{}".format(i+1)) )}')
                print(20*'-')
    ...
    # -------------------------------------------------------------------------------- solve
    def solve(self, output=True, show_calculations=False):
        """Solves the beam equilibirum problem, determining the reactions, diagrams of
        internal loads and deflections.
        """
        # Check if the properties have been properly set.
        self._check_beam_properties()
        # Set the beam segments with piecewise continuous properties.
        self._set_segments(print_segments=show_calculations)
        # Solve for the exterior reactions.
        self._solve_reactions(show_calculation=show_calculations)
        # Solver for internal loads.
        self._solve_internal_loads(show_calculation=show_calculations)
    ...
    def _print_reactions(self):
        ...
        for ipoint in self.points:
            if ipoint.has_reaction_force():
                ...
                print(
                    "{0:^27} {1:^27} {2:}".format(
                        x_coord_str, "Force", ipoint.reaction_force
                    )
                )
            ...
            if ipoint.has_reaction_moment():
                ...
                print(
                    "{0:^27} {1:^27} {2:^27}".format(
                        x_coord_str, "Moment", ipoint.reaction_moment
                    )
                )
...
class _segment:
    ...
    def __str__(self) -> str:
        ...
        return (
            "Segment: "
            + str(self.x_start.round(2))
            + " - "
            + str(self.x_end.round(2))
            + " (Cut at: "
            + str(((self.x_start + self.x_end) / 2).round(2))
            + ")"
        )
